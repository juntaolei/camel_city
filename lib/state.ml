...
let canvas_width state = fst (canvas_size state)
let canvas_height state = snd (canvas_size state)
let map_length state = state.map_length
let cell_size state = state.cell_size
let cell_width state = fst (cell_size state)
let cell_height state = snd (cell_size state)
...
let rec resource_sufficiency_check_helper (dependencies:resource list) (resource_stockpile:stockpile) :bool =
...
let resource_sufficiency_check (building:building) (resource_stockpile:stockpile) : bool =
...
let rec sub_resource_helper (dependencies:resource list) (resource_stockpile:stockpile) : stockpile = 
 match dependencies with
    | [] -> resource_stockpile
    | h::t -> let resource_name = Buildings.resource_name h in
    let resource_change = Buildings.resource_amount h in
...
let sub_resource (building:building) (resource_stockpile:stockpile) : stockpile = 
  let dependencies = Buildings.resource_dependency building in
...
let sub_maintance (building:building) (resource_stockpile:stockpile) : stockpile = 
...
let add_income (building:building) (resource_stockpile:stockpile) : stockpile =
...
let add_resources (building:building) (resource_stockpile:stockpile) : stockpile = 
...
let rec update_resources (building_list: building list) (resource_stockpile:stockpile) : stockpile = 
  match building_list with
  | [] -> resource_stockpile
  | h::t -> if not (resource_sufficiency_check h resource_stockpile) then update_resources t (sub_maintance h resource_stockpile)
...
let rec next_state (state:state) : state  = 
  if pause then 
    if state.tick < 100 then 
      let new_resources = update_resources state.buildings state.stockpile in
      let update_state = new_state (tick state + 1) (canvas_width state) (canvas_height state)
       (map_length state) (cell_width state) (cell_height state) new_resources (get_buildings state) in
