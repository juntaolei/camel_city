...
let update_game_over_text state =
  let message =
    ...
    if cond = 1 then
      "You have taken too long and the camel people have had enough of \
       your leadership and they have all abandoned you."
    else if cond = 2 then
      "Without food your people have starved to death. Camel City now \
       lies abandoned; littered with skeletons"
    else if cond = 3 then
      "Your city is bankrupted. In desperate need to pay off the debt, \
       you sold Camel City to the nation of Java Cafe, whose upper \
       classes use it as a source of cheap exploitable labor."
    else if state.is_final_building_placed then
      "By building the wonder you have resurected the great camel \
       prophet, The Clarkson. He shall now lead your people to victory \
       against the Python Empire. You have won and saved camelkind"
    else ""
  in
