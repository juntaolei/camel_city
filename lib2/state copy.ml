...
let revolt (revolt_counter:int) (revolt_limit:int) = if happy_counter < revolt_limit then false else true
...
let alavilble_pop (population:int) (buildings: building list) =
  population - working_pops buildings
...
let next_state (state : state) : state =
  let a = starve_count in
  let is_starved = in
  let b = broke_count in
  let is_broke = in
  let c = 
