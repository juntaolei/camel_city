...
(* This test suite covers the most basic commands in [state.ml] including 
    functions for creating and updating states. *)

let state_0 = new_state "test_state.json" 1000 900 10 128 64
let state_1 = from_file "map_1.json"

(** [expected_cell_array_1] is the expected cell array array extracted from
    file "map_1.json". *)
...
(** [lst_of_array_array a] is the cell list list represented by the cell array
    array [a]. *)
...
(** [string_of_lst acc lst] is the string representation of cell list [lst]. *)
let rec string_of_lst acc = function
  | [] -> acc
  | h :: t -> string_of_lst (acc ^ str_of_cell h ^ ",") t

(** [string_of_lst acc lst] is the string representation of cell list list
    [lst]. *)
let rec string_of_lst_lst acc = function
  | [] -> acc
  | h :: t -> string_of_lst_lst ("[" ^ string_of_lst "" h ^ "]" ^ acc) t

(** [string_of_stockpile acc sp] is the string representation of stockpile 
    [sp]. *)
let rec string_of_stockpile acc = function
| [] -> acc
| h :: t -> 
  string_of_stockpile 
    (acc ^ (resource_name h) ^ ":" ^ (string_of_int (resource_amount h))) t
...
let stockpile_test name st expected : test =
  ...
  ~printer: (fun x -> string_of_stockpile "" x)
