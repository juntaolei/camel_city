...
let expected_cell_array = 
  ...
  empty.(0).(0) <- Building (new_building "mine" 0 0 1 "iron" 0 0 5 "electricity");
  empty.(0).(1) <- Building (new_building "house" 0 0 0 "" 0 0 0 "");
  empty.(2).(0) <- Building (new_building "oats_plantation" 0 0 10 "oat" 0 0 0 "");
  empty
...
let canvas_size_test name st expected : test = 
  name >:: fun _ -> 
  assert_equal expected (canvas_size st) 
  ~printer:(fun (a, b) -> (string_of_int a) ^ (string_of_int b))

let cell_size_test name st expected : test = 
  name >:: fun _ -> 
  assert_equal expected (cell_size st) 
  ~printer:(fun (a, b) -> (string_of_int a) ^ (string_of_int b))
...
let cell_test name st expected : test =
  name >:: fun _ -> 
  assert_equal expected (cells st) 
  ~printer: (fun x -> string_of_lst_lst "" (lst_of_array_array x))

let stockpile_test name st expected : test =
  name >:: fun _ -> 
    assert_equal expected (stockpile st) 

let state_test =
  [
    canvas_size_test "canvas size of state is 1000 by 900" state (1000,900);
    map_length_test "map length of state is 10" state 10;
    cell_size_test "cell size of state is 128 by 64" state (128,64);

    canvas_size_test "canvas size of state_1 is 300 by 300" state_1 (300,300);
    map_length_test "map length of state_1 is 3" state_1 3;
    cell_size_test "cell size of state_1 is 100 by 100" state_1 (100,100);
